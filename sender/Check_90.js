...
let count_time = setInterval(function () {
    if (keep_time[pose_status] == 0) {
        ...
        if (pose_status == 0){
            window.parent.postMessage({message: `왼쪽 팔을 ${keep_time[pose_status]}초 뻗었습니다..`}, "*");
            //window.alert("left");
            const spawn = require('child_process').spawn;
            ...
            window.parent.postMessage({message: `오른쪽 팔을 ${keep_time[pose_status]}초 뻗었습니다.`}, "*");
            //window.alert("right");
            const spawn = require('child_process').spawn;
            const result = spawn("python3", ['send_right.py']);
        }
        else if(pose_status == 2){ 
            window.parent.postMessage({message: `기본 자세입니다.`}, "*");
            }
...
/*function check_X(pose) {
    ...
    if (b.y > le.y && b.y > re.y && le.y > lw.y && re.y > rw.y && lw.y > head.y && rw.y > head.y) {
        ...
        if (rs.x < rw.x || lw.x < ls.x) {
            r_gradient = -1;
            l_gradient = 1;
            if (rw.x - re.x != 0) { //손목값에서 팔꿈치 값을 뺀다(x표에서 음의 좌표 = 오른쪽 팔의 움직임 고려)
                r_gradient = (rw.y - re.y) / (rw.x - re.x);
            }
            if (lw.x - le.x != 0) { //손목값에서 팔꿈치 값을 뺀다(x표에서 양의 좌표 = 왼쪽 팔의 움직임 고려)
                l_gradient = (lw.y - le.y) / (lw.x - le.x);
            }
            if (r_gradient < 0 || l_gradient > 0) { //둘중 하나라도 만족한다면, 정답으로 가정
                return true;
            } else {
                return false;
            }
